...
const POOL_KEY = "pool-key-abcdef"
...
test("lease sends bearer key and parses 200 body", async () => {
  ...
  // Then: the parsed lease comes back, and the request carried the pool key + the spent id
  expect(out).toEqual({ ok: true, lease })
  expect(calls).toHaveLength(1)
  expect(calls[0].url).toBe(`${MASTER}${CLOUD_ROUTES.lease}`)
  expect(calls[0].init.method).toBe("POST")
  expect(headersOf(calls[0]).Authorization).toBe(`Bearer ${POOL_KEY}`)
...
test("maps 401 to typed auth error", async () => {
  // Given: the master rejects this worker's pool key
  const { impl, calls } = fakeFetch(async () => new Response(JSON.stringify({ error: "unknown pool key" }), { status: 401 }))
  const { delays, sleep } = sleepRecorder()
  const client = createLeaseClient({ fetchImpl: impl, sleep, masterUrl: MASTER, poolKey: POOL_KEY, workerId: WORKER_ID })
  ...
  // Then: a decisive answer — surfaced as-is, with NO retry (the key will not become valid)
  expect(out).toEqual({ ok: false, failure: { kind: "auth" } })
  expect(calls).toHaveLength(1)
  expect(delays).toEqual([])
})
...
test("ratelimit report posts diagnostic headers", async () => {
  ...
  const base = { sleep, masterUrl: MASTER, poolKey: POOL_KEY, workerId: WORKER_ID }
  ...
  expect(headersOf(accepted.calls[0]).Authorization).toBe(`Bearer ${POOL_KEY}`)
