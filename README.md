...
**可以同时开多个会话**,每个会话占一个槽位、用一个独立的 workerId(`vince-cc.1` / `.2` …)向 master 租号。这一点是必须的:master 的持有者账本按 workerId 键,同一个标签下的第二次租约会覆盖第一条——于是两个会话可能被派到**同一个账号**,一个 5 小时窗口被双倍烧。一个会话一个标签,`fewestHolders` 才算得对,同机器上 senpi 那条链也会自动绕开我们占着的号。
...
并发位占满时启动会以 **75** 退出并说明原因,而不是不占槽位硬跑——那样这个会话对别的会话不可见,master 也不会把它记成独立持有者,正是双占的来源。槽位靠文件锁持有整个会话;进程被 `kill -9` 时锁会在约一分钟后失效并被回收。
