...
- `/v1/health` 是**免鉴权**的就绪探针,两个 POST 端点和 `/v1/usage` 都要求 `Authorization: Bearer <poolKey>`;请求体里的 `workerId` 只作参考,**身份一律由 key 决定**(信任请求体等于可被伪造)。
...
| 路由 | 鉴权 | 内容 |
|---|---|---|
| `/v1/usage` | 需要 pool key | 全池每个 anthropic 账号的 label、id 前缀、各窗口利用率与 `resetsAt`、冷却状态、token 到期、`needsReauth` / `excluded` |
| `/` | **免鉴权** | 一个零依赖的 HTML 页面。它**不含任何数据**,只是在浏览器里问你要 pool key(存 sessionStorage),再自己带 `Authorization` 头去拉 `/v1/usage` |

几条实现上的硬约束:

- **绝不输出 token。** 载荷类型里根本没有能放 `access` / `refresh` 的字段,想泄露必须先加字段——那正是 review 会看到的地方。账号 id 只给前 8 位。
- **不新增任何对外部 API 的轮询。** 看板读的是 `usagePoller` 已经在跑的那份快照,所以刷新页面**不可能**触发 `/api/oauth/usage`(它的 429 会持续好几分钟,且按出口 IP 连坐)。
