...
>
> 上一份报告给出了「推荐 A」的结论，但逐方案分析只有一张四行表。本文补足每个方案的**机制阻塞点、工程量、失败模式、维护成本、合规定性**，并纠正上一份报告中的两处事实错误和一处伪造引用。
>
...
### 0.1 【严重】伪造引用：核心论据引用了一个不存在的文件

`pool-feasibility.md` 四次引用 `docs/账号token迁移到新电脑操作指南.md`，其中包括**整个可行性判断的地基**：

> §2.1「实证来自 `docs/账号token迁移到新电脑操作指南.md`：把账号 token 搬到**另一台电脑**、在新机刷新后，直接 `curl .../api/oauth/usage` 返回 200（§3 步骤 5）。」

**核实结果：该文件不存在。**

```
claude-accounts-usage/docs/  →  只有 2 个文件
  claudecode-usage-查询机制分析.md
  ex-machina-源码机制分析.md
git log --diff-filter=A -- 'docs/*'  →  只有一次提交（32da7e4），新增上述 2 个文件
find . -iname "*迁移*" -o -iname "*新电脑*"  →  无结果
```

`claude-accounts-usage` 整个代码库中**从未有任何实验测试过跨机器使用 access token**。该仓库的全部设计空间都是「单机多进程」，作者根本没考虑过这个问题。所谓「三条铁律 §4」同样出自这个不存在的文件。

**影响与处置**：
- 「铁律 1（refresh 轮换）」「铁律 2（双机互顶锁死）」**结论本身正确**，但真实出处是 `claudecode-usage-查询机制分析.md §3`（引 sub2api Issue #1035）+ `.omo/drafts/auth-file-lock-multi-instance.md:38` 的真实生产事故日志（毫秒级时间戳）+ `anthropics/claude-code#43392`。引用必须改。
- 「access token 跨机可用」**结论也正确，但原引用完全无效**。真实依据见 §1.2：34.5k★ 的 sub2api、12.4k★ 的 claude-relay-service 就是在机房 IP 上大规模做这件事，这是比任何单次 curl 都强的证据。

> 教训：本项目后续所有报告，引用必须可回溯到真实存在的 `文件:行号` 或 URL。本文所有论断均附可核验出处。
...
这比上一份报告里那个不存在的「单次 curl 实验」强出几个数量级：**access token 不绑 IP、不绑机器**，否则这些项目一天都活不下去。
...
| 「中心刷新 + 另一 IP 用 access 打 inference 完全无冲突」 | 未做过定向实验。§1.2 的间接证据很强，但没有我们自己的一手验证。 |
...
### 实验 2 access token 跨 IP 可用性（补上历史欠账）

上一份报告声称已实证，实际引用了不存在的文件（§0.1）。真实状态是：有 34.5k★ 生产项目的强间接证据，但**我们自己没有一手验证**，而整个池子的地基压在这条上。
